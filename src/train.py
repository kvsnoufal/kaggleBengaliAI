...
class CustomCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if avg_result == max(self.avg_recall):
            ...
            self.model.save_weights(f'../log/best_avg_recall_{TEST_FOLDS[0]}.h5')
        return
...
if __name__ == "__main__":
    ...
    train_gen=BengaliData(folds=TRAIN_FOLDS,img_height=137,img_width=236,mean=(0.485,0.456,0.406),std=(0.229,0.224,0.225),batch_size=BATCH_SIZE)
    valid_gen=BengaliData(folds=TEST_FOLDS,img_height=137,img_width=236,mean=(0.485,0.456,0.406),std=(0.229,0.224,0.225),batch_size=BATCH_SIZE)
    ...
    chkPoint = ModelCheckpoint(f'../log/train_{TEST_FOLDS[0]}.h5',
    ...
    earlyStop = EarlyStopping(monitor='val_root_loss',
                            ...
                            min_delta = 0,
                            verbose = 2)
    csv_logger = CSVLogger(f'../log/csvLog_{TEST_FOLDS[0]}.csv')
    ...
    with open(f'../log/trainHistoryDict_{TEST_FOLDS[0]}.p', 'wb') as file_pi:
